...
class RecordDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.team_edit.setPlaceholderText('团牌（可选）')
        form_layout.addRow('团牌:', self.team_edit)

        # 掉落
        self.loot_edit = QLineEdit()
        self.loot_edit.setPlaceholderText('掉落（可选）')
        form_layout.addRow('掉落:', self.loot_edit)
    ...
    def _load_record(self):
        # record: (id, date_range, name, faction, ns, nc, hs, hc, total, team, loot)
        date_range = self.record[1] if len(self.record) > 1 else ''
        name = self.record[2] if len(self.record) > 2 else ''
        faction = self.record[3] if len(self.record) > 3 else ''
        team = self.record[9] if len(self.record) > 9 else ''
        loot = self.record[10] if len(self.record) > 10 else ''
        n_sal = self.record[4] if len(self.record) > 4 else 0
        n_con = self.record[5] if len(self.record) > 5 else 0
        h_sal = self.record[6] if len(self.record) > 6 else 0
        h_con = self.record[7] if len(self.record) > 7 else 0
        ...
        self.team_edit.setText(team)
        self.loot_edit.setText(loot)
    ...
    def _on_save(self):
        ...
        loot = self.loot_edit.text().strip()
        n_sal = parse_brick_input(self.normal_salary_edit.text())
        n_con = parse_brick_input(self.normal_consume_edit.text())
        h_sal = parse_brick_input(self.hero_salary_edit.text())
        h_con = parse_brick_input(self.hero_consume_edit.text())
        ...
        self.result_data = (date_range, start_str, end_str, name, faction,
                            n_sal, n_con, h_sal, h_con, team, loot)
        self.accept()
